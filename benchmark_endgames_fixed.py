...
def main():
    ...
    ap.add_argument('--only',choices=['all','queen','rook'],default='all')
    args=ap.parse_args()
    ...
    if args.only!='all':
        prefix='KQ_' if args.only=='queen' else 'KR_'
    ...
    results=[];errors=[]
    print(f'{len(tasks)}个残局；v4执优势方；Stockfish防守。结果：',folder,flush=True)

...
def main():
    ...
    path=ROOT/'neural_speed_v2_report.json'
    def save():
    ...
    report['comparison']='v1 versus v2; same model and input encoder'
    ...
    print('编码及专项和棋核验通过。开始v1/v2同深度测试。',flush=True)
    def run(kind,b,depth,seconds):
        before=(b.fen(),list(b.move_stack))
        engine=(FastSearchEngine(old,seconds=seconds,max_depth=depth,claim_draw=True) if kind=='old'
    ...
    report['profiles']={}
    for kind in ['old','fast']:
        profiler=cProfile.Profile()
        profiler.enable()
        run(kind,boards[1],2,120)
        profiler.disable()
        entries=[]
        for (file,line,name),(primitive,calls,own,cumulative,callers) in pstats.Stats(profiler).stats.items():
            entries.append({'function':name,'file':file,'line':line,'calls':calls,'own_seconds':own,'cumulative_seconds':cumulative})
        report['profiles'][kind]=sorted(entries,key=lambda x:x['cumulative_seconds'],reverse=True)[:35]
    save()

...
from search_engine import policy_order, SearchEngine
...
OPENINGS += [
    ("彼得罗夫", ["e4","e5","Nf3","Nf6","Nxe5","d6"]),
    ("维也纳", ["e4","e5","Nc3","Nf6","Bc4","Nc6"]),
    ("四马", ["e4","e5","Nf3","Nc6","Nc3","Nf6"]),
    ("菲利多尔", ["e4","e5","Nf3","d6","d4","Nf6"]),
    ("西西里封闭", ["e4","c5","Nc3","Nc6","g3","g6"]),
    ("法兰西推进", ["e4","e6","d4","d5","e5","c5"]),
    ("卡罗康推进", ["e4","c6","d4","d5","e5","Bf5"]),
    ("尼姆佐印度", ["d4","Nf6","c4","e6","Nc3","Bb4"]),
    ("后印度", ["d4","Nf6","c4","e6","Nf3","b6"]),
    ("荷兰", ["d4","f5","g3","Nf6","Bg2","g6"]),
]
...
def run_pair(args):
    ...
    engines={
        ...
        "material":SearchEngine(seconds=SECONDS,max_depth=DEPTH,claim_draw=True),
    }
    ...
    manifest = {
        ...
        "scope": "v4对原版material，同策略排序、同线程与时间；未证明这些开局未用于训练。"
    }
    (folder/"config.json").write_text(json.dumps(manifest,ensure_ascii=False,indent=2),encoding="utf-8")

    def save_summary():
        summary["finished_games"] = summary["v4_wins"] + summary["material_wins"] + summary["draws"] + summary["unfinished"]
    ...
    for opening_name, opening_moves in [OPENINGS[args.pair]]:
        for neural_is_white in [True, False]:
            ...
            white = "v4" if neural_is_white else "material"
            black = "material" if neural_is_white else "v4"
    ...
    print("material获胜：", summary["material_wins"])
...
def main():
    root=Path(__file__).resolve().parent
    ap=argparse.ArgumentParser(description="40局：v4 vs material；5秒，最高深度5")
    ...
    if args.pair is not None:
        run_pair(args)
        return
    if not 1<=args.workers<=4:ap.error("workers须为1至4")
    ...
    if hashlib.sha256(args.model.read_bytes()).hexdigest()!='4e44421b422c8467a32c06c85a31a5591b8397ae24c2f54fdd39c778fe7dd4a0':
        ap.error('请选择稳定的全量第2轮权重，不能使用残局微调模型')
    ...
    output=root/"arena_v4_material40_games"/datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    ...
    config={"v4":str(args.model),"v4_sha256":hashlib.sha256(args.model.read_bytes()).hexdigest(),
            ...
            "search_sha256":hashlib.sha256((root/"search_engine.py").read_bytes()).hexdigest(),
            "workers":args.workers,"threads_per_worker":4,"games":40,"seconds":SECONDS,"depth":DEPTH,
    ...
    print(f"40局；同时最多{args.workers}局；每进程4线程；5秒/步，最高深度5",flush=True)
    ...
    try:
        while next_pair<len(OPENINGS) or active:
            while next_pair<len(OPENINGS) and len(active)<args.workers:
            ...
            for i,(proc,log) in list(active.items()):
                ...
                if code is not None:
                    ...
                    print(f"开局组{i+1}结束，退出码{code}；已结束{len(completed)+len(failed)}/20组",flush=True)
            time.sleep(.25)
    except KeyboardInterrupt:
        interrupted=True
        ...
        for proc,log in active.values():
            ...
            log.close()
        summary={"v4_wins":0,"material_wins":0,"draws":0,"unfinished":0,
        ...
        for p in output.glob("pair_*/summary.json"):
            ...
            for k in ["v4_wins","material_wins","draws","unfinished"]:summary[k]+=part[k]
        summary["accounted_games"]=sum(summary[k] for k in ["v4_wins","material_wins","draws","unfinished"])
        summary["unaccounted_games"]=40-summary["accounted_games"]

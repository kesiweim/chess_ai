...
def main():
    ...
    path=ROOT/'neural_speed_v4_report.json'
    def save():
    ...
    report['comparison']='v3 versus v4 ordering; same traced evaluator; different node counts expected'
    ...
    for b in special:
        ...
        e=NeuralSearchV4(fast,seconds=3,max_depth=2,claim_draw=True,verify_claims=True)
    ...
    for i,b in enumerate(boards[:12]):
        ...
        same=x.get('score')==y.get('score')
        same_move=x['move']==y['move']
        completed=all(r['mate_one'] or r['depth']==2 for r in results.values())
        report['fixed_depth'].append({'case':i,**results,'same_score':same,'same_move':same_move,'completed':completed});save()
        print(f"固定深度 {i+1}/12：评分一致={same}，选招一致={same_move}；旧{x['wall_seconds']:.3f}s，新{y['wall_seconds']:.3f}s",flush=True)
        if not same or not completed:
            report['eligible']=False;save();print('固定深度检查未通过，此配置不推荐。',flush=True);return
    report['eligible']=True
    report['scope']='实验版；排序改变节点数；固定深度评分检查不等于棋力认证。'
    save()

...
def main():
	...
	# 检查纯模型对一步将死走法的评分
        for move in mating_moves:
            ...
            rank = int(
                (legal_scores > legal_scores[index]).sum().item()
            ) + 1

            print(
                f"\n将死走法 {board.san(move)} ({move.uci()})："
                f"排名 {rank}/{len(legal_moves)}，"
                f"概率 {probabilities[index].item():.4%}"
            )

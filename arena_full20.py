...
def run_pair(args):
    ...
    print("交换颜色两局；双方CPU1线程，每步5秒、深度上限5。", flush=True)
    ...
    manifest = {
        ...
        "threads": 1, "max_plies": MAX_PLIES, "openings": OPENINGS,
...
def main():
    root=Path(__file__).resolve().parent
    ap=argparse.ArgumentParser(description="20局：全量第2轮 vs material；5秒，最高深度5")
    ...
    if args.model is None:
        ...
        if not candidates:ap.error("找不到全量第2轮模型；请用 --model 指定")
    ...
    output=root/"full_arena_games"/datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    output.mkdir(parents=True)
    config={"candidate":str(args.model),"candidate_sha256":hashlib.sha256(args.model.read_bytes()).hexdigest(),
            "policy_sha256":hashlib.sha256((root/"chess_model_balanced.pt").read_bytes()).hexdigest(),
            "search_sha256":hashlib.sha256((root/"search_engine.py").read_bytes()).hexdigest(),
            "workers":args.workers,"threads_per_worker":1,"games":20,"seconds":SECONDS,"depth":DEPTH,
    ...
    print(f"20局；同时最多{args.workers}局；每进程1线程；5秒/步，最高深度5",flush=True)
    ...
    try:
        ...
        for proc,log in active.values():
            ...
            log.close()
        summary={"candidate_wins":0,"material_wins":0,"draws":0,"unfinished":0,
        ...
        for p in output.glob("pair_*/summary.json"):
            ...
            for k in ["candidate_wins","material_wins","draws","unfinished"]:summary[k]+=part[k]
        summary["accounted_games"]=sum(summary[k] for k in ["candidate_wins","material_wins","draws","unfinished"])

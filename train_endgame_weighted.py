import argparse,json,random,hashlib,time,math
...
def main():
    ...
    ap=argparse.ArgumentParser();ap.add_argument('--model',type=Path);ap.add_argument('--epochs',type=int,default=3);ap.add_argument('--endgame-weight',type=float,default=0.5);args=ap.parse_args()
    if not math.isfinite(args.endgame_weight) or args.endgame_weight<=0:ap.error('endgame-weight必须为有限正数')
    ...
    if args.model is None:
        ...
        args.model=files[-1]
    expected='4e44421b422c8467a32c06c85a31a5591b8397ae24c2f54fdd39c778fe7dd4a0'
    if hashlib.sha256(args.model.read_bytes()).hexdigest()!=expected:
        raise ValueError('模型不是上次试验的基础权重，请用--model指定原全量第2轮模型')
    print('基础模型：',args.model, '；残局系数：',args.endgame_weight,flush=True)
    ...
    folder=ROOT/'endgame_weighted_runs'/datetime.now().strftime('%Y%m%d_%H%M%S_%f');folder.mkdir(parents=True)
    write(folder/'data_report.json',{'train':len(train),'val':len(val),'replay':len(replay),'oldval':len(oldval),
        ...
        'epochs':args.epochs,'lr':1e-5,'batch_endgame':128,'batch_replay':128,'old_loss_weight':1.0,'endgame_loss_weight':args.endgame_weight,'device':str(device),
    ...
    try:
        for epoch in range(1,args.epochs+1):
            ...
            for i in range(0,len(train),128):
                ...
                loss=args.endgame_weight*(p[:len(new)]-y[:len(new)]).square().mean()+(p[len(new):]-y[len(new):]).square().mean()
